    ...
    public static void main(String[] args) {
        ...
        for (String j : phonebook.keySet()) {
            if (phonebook.containsValue(phonebook.get(j))) {
                newphonebook.replace(phonebook.get(j), newphonebook.get(phonebook.get(j)),
                        newphonebook.get(phonebook.get(j)) + "; " + j);
            }
        }
        for (String i : newphonebook.keySet()) {
            newphonebook.replace(i, newphonebook.get(i),
                    newphonebook.get(i).substring(5, newphonebook.get(i).length()));
        }
